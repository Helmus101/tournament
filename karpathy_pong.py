"""Train karpathy_pong with a stronger both-side curriculum on pong_v3."""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train karpathy_pong.")
...
def main():
    ...
    if os.path.exists(SAVE) and not args.fresh:
        ...
        print(f"fresh karpathy_pong -> {SAVE.name}")

    recent = deque(previous_recent[-gate_window:], maxlen=gate_window)
    env = pong_v3.parallel_env(max_cycles=MAX_CYCLES)
    opt.zero_grad()
    start_episode = episode
    print("training karpathy_pong: both sides, curriculum random -> mixed -> follower. Ctrl-C to stop.")
