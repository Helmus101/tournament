...
SPEED is UNPREDICTABLE and NOT physics-based:
  ...
  * the ball also CURVES: a small random vertical drift every step bends its path
    (a banana-ball), so even between jolts it does not travel in a straight line.
...
ball will land. This is the "harder arena". Tune difficulty via the constants
below (CHAOS_SPEED_MAX, JOLT_PROB, CURVE_STD).
...
# ── chaos parameters (the difficulty knobs -- raise these to make it harder) ──
...
CHAOS_SPEED_MAX = 6.0     # ball-speed ceiling -- harder than the standard 5.0 cap, still
                          # under the field width so collisions can't tunnel (keep <= ~6.5)
JOLT_PROB       = 0.08    # per-step chance the ball's speed suddenly jumps mid-flight
CURVE_STD       = 0.30    # per-step random vertical drift -> the ball CURVES unpredictably
                          # (0 = straight lines between hits; bigger = wilder banana-balls)
    ...
    def step(self, a_right, a_left):
        ...
        if self.rng.random() < JOLT_PROB:
            ...
            if cur > 1e-6:
                ...
                self.bvx *= scale
                self.bvy *= scale
        # curveball: a small random vertical drift every step makes the ball arc on a path
        # you can't extrapolate (bvx is left alone, so horizontal collisions still can't tunnel).
        if CURVE_STD > 0:
            self.bvy = float(np.clip(self.bvy + self.rng.normal(0.0, CURVE_STD),
                                     -CHAOS_SPEED_MAX, CHAOS_SPEED_MAX))
...
def _has_positional_models():
    """True if the user named at least one entrant on the CLI (vs only flags). Skips the
    value that follows a value-taking flag (--best-of N, --seed N) so it isn't mistaken
    for a model name."""
    value_flags = {"--best-of", "--seed"}
    skip = False
...
def main():
    ...
    print(f"  + curveball: vertical drift std {CURVE_STD}/step (set CURVE_STD=0 to disable)")
    print("=" * 56)
